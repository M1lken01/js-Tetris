...
function move(direction = [0, 0], rotate = false) {
  ...
  for (let i = 0; i < game.shape.shape.length; i++) {
    let col = willCollide(
      game.moving[0] + game.shape.shape[i][0],
      game.moving[1] + game.shape.shape[i][1],
      game.moving[0] + game.shape.shape[i][0] + direction[0],
      game.moving[1] + game.shape.shape[i][1] + direction[1],
    );
    if (col[0] != 0) {
      //game.moving[0] += direction[0];
      xCanMove = false;
...
function willCollide(x, y, xOff, yOff) {
  r = [1, 1];
  if (-xEdge <= xOff && xOff <= xEdge && game.stable[getHeight(y)][xOff + xEdge] == 0) r[0] = 0;
  if (yOff <= yEdge && game.stable[getHeight(yOff)][x + xEdge] == 0) r[1] = 0;
  return r;
}
...
function stabilize() {
  for (let i = 0; i < game.shape.shape.length; i++) {
    let x = getHeight(game.moving[1] + game.shape.shape[i][1]);
    let y = game.moving[0] + xEdge + game.shape.shape[i][0];
    game.stable[x][y] = 1;
    game.colormap[x][y] = game.shape.color;
  }
  game.moving = [0, -5];
  game.shape = getShape();
}
...
function gameLoop() {
  console.log('loop');
  if (!paused) move(directions.down);
